...
# reading the data ...
x,y,z,t,data = openmhd.data_read("data/field-00020.dat")
# reading the data (partial domain: [ix1,ix2] x [jx1,jx2] x [kx1,kx2])
# x,y,z,t,data = openmhd.data_read("data/field-00020.dat",ix1=0,ix2=100,jx1=11)
...
# az[0,0] = 0.0
az[0,-1] = 0.5*(data[0,-1,k,bx] - data[0,-1,k,by])
for j in range(y.size-1,0,-1):
    az[0,j-1] = az[0,j] - 0.5*(data[0,j-1,k,bx]+data[0,j,k,bx])
for i in range(1,x.size):
    az[i,:] = az[i-1,:] - 0.5*(data[i-1,:,k,by]+data[i,:,k,by])
