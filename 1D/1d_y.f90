...
!-----------------------------------------------------------------------
  ...
  real(8) :: G(ix,jx,var1)   ! numerical flux (F,G)
!-----------------------------------------------------------------------
  integer :: j, k
  integer :: n_output
...
!       write(6,*) 'U* = U + (dt/dx) (F-F)'
        do j=2,jx-1
           U1(1,j,:) = U(1,j,:) + dtx*( G(1,j-1,:)-G(1,j,:) )
        enddo
     elseif( time_type .eq. 1 ) then
!       write(6,*) 'U*(n+1/2) = U + (0.5 dt/dx) (F-F)'
        do j=2,jx-1
           U1(1,j,:) = U(1,j,:) + 0.5d0*dtx*( G(1,j-1,:) - G(1,j,:) )
        enddo
     endif
! --- BC ---
     U1(:,jx,:) = U1(:,jx-1,:)
...
!       write(6,*) 'U_new = 0.5( U_old + U* + F dt )'
        do j=2,jx-1
           U(1,j,:) = 0.5d0*( U(1,j,:)+U1(1,j,:) + dtx*( G(1,j-1,:)-G(1,j,:) ) )
        enddo
     elseif( time_type .eq. 1 ) then
!       write(6,*) 'U_new = U + (dt/dx) (F-F) (n+1/2)'
        do j=2,jx-1
           U(1,j,:) = U(1,j,:) + dtx*( G(1,j-1,:) - G(1,j,:) )
        enddo
     endif
! --- BC ---
     U(:,jx,:) = U(:,jx-1,:)
