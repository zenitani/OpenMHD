subroutine mpioutput(filename,ix,jx,t,x,y,U,V,myrank,npe)
  ...
  call mpi_file_write_all( fh,  t, 1,   mpi_real8, mpi_status_ignore, ierr) ; disp = disp+8
  call mpi_file_write_all( fh,iix, 1, mpi_integer, mpi_status_ignore, ierr) ; disp = disp+4
  call mpi_file_write_all( fh, jx, 1, mpi_integer, mpi_status_ignore, ierr) ; disp = disp+4

  call mpi_file_set_view(fh, disp+8*myrank*(ix-2), mpi_real8, mpi_real8, "native", mpi_info_null, ierr)
  call mpi_file_write_all( fh, x, ix, mpi_real8, mpi_status_ignore, ierr) ; disp = disp+8*iix
  call mpi_file_set_view(fh, disp, mpi_byte, mpi_byte, "native", mpi_info_null, ierr)
  call mpi_file_write_all( fh, y, jx, mpi_real8, mpi_status_ignore, ierr) ; disp = disp+8*jx
