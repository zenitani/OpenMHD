...
!    vfL = sqrt( ( (f1+B2) + sqrt( (f1+B2)**2 - f1*f2 )) / ( 2*VL(i,j,ro) ))
     ...
     ptL = VL(i,j,pr) + 0.5d0*B2
!    LHLLD
     aL1 = ( (v2+B2) + sqrt(max( (v2+B2)**2 - v2*f2, 0.d0 ))) / ( 2*VL(i,j,ro) )
...
!    vfR = sqrt( ( (f1+B2) + sqrt( (f1+B2)**2 - f1*f2 )) / ( 2*VR(i,j,ro) ))
     ...
     ptR = VR(i,j,pr) + 0.5d0*B2
!    LHLLD
     aR1 = ( (v2+B2) + sqrt(max( (v2+B2)**2 - v2*f2, 0.d0 ))) / ( 2*VR(i,j,ro) )

!    pressure correction factor phi (f2)
     f1 = min( 1.d0, max( aL1, aR1 ) / max( aL, aR ) )  ! phi**2
     f2 = 2 * sqrt(f1) - f1  ! phi
