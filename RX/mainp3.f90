...
!-----------------------------------------------------------------------
  ...
  integer, parameter :: ix =   32        ! 30 x 600 --> 18000 cells
  integer, parameter :: jx = 1202
  integer, parameter :: loop_max = 1000000
  real(8), parameter :: tend  =  2000.0d0
...
! Resistivity
  real(8), parameter :: Rm1 = 1000.d0, Rm0 = 1000.d0
!-----------------------------------------------------------------------
...
! We calculate dx here
  ...
  call mpibc3(U,ix,jx,myrank,npe)
...
999  format (' limiter: ', i1, '  flux: ', i1, '  time-marching: ', i1 )
  ...
  if ( n_start .ne. 0 ) then
     ...
     call input(filename,ix,jx,t,x,y,U,V)
  endif
  t_output = t - dt/3.d0
  n_output = 0
...
!     write(6,*) 'fix VL/VR at MPI boundary'
     call mpibc_vlvr_f3(VL,VR,ix,jx,myrank,npe)
...
!    fix flux bc (G)
     call mpibc_vlvr_g3(VL,VR,ix,jx,myrank,npe)
...
!    boundary condition
     call mpibc3(U1,ix,jx,myrank,npe)
!     write(6,*) 'U* --> V'
     call u2v(U1,V,ix,jx)
